...
func (s *StorageView) List(ctx context.Context, prefix string) ([]*StorageEntry, error) {
...
func (s *StorageView) ExpandKey(suffix string) string {
	return s.prefix + StorageKeySep + suffix
}

// truncateKey is used to remove the prefix of the key
func (s *StorageView) TruncateKey(full string) string {
	return strings.Join(strings.Split(full, StorageKeySep)[1:], StorageKeySep)
}
